...
import { canAccess, ROLES } from '../utils/roles';
...
export default function Sidebar() {
  ...
  const ALL_NAV_ITEMS = [
    ...
    { to: '/admin',         label: 'Admin',                 icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg> },
  ];
  ...
  const NAV_ITEMS    = ALL_NAV_ITEMS.filter(item => canAccess(role, item.to));
  const BOTTOM_ITEMS = BOTTOM_ALL.filter(item => canAccess(role, item.to));
    ...
    <aside style={{
      ...
      <div style={{ marginTop: '8px', paddingTop: '12px', borderTop: `1px solid ${theme.border}` }}>
        <div style={{ padding: '4px 12px 8px', fontSize: '11px', color: theme.textSecondary }}>
          ...
          <div style={{ marginTop: '2px', color: theme.textMuted }}>{ROLES[user?.role]?.label ?? user?.role}</div>
        </div>
